...
\title{Задачи по Анализу временных рядов}
...
\begin{align*}
	\Phi(L)(1-L)^dy_t&=\Theta(L)u_t & u_t&\sim WN(0,\sigma^2)
\end{align*}
для следующих многочленов
\begin{center}
	\begin{tabular}{l|c|c|c}
		\textnumero & \(d\) & \(\Phi(z)\) & \(\Theta(z)\) \\ \hline
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\begin{center}\small
		\begin{tabular}{l|c|c}
			Модель & trend  & спецификация\\ \hline
			ARIMA(1,0,1) & \texttt{'ct'} & \(y_t=\alpha_0+\alpha_1t+\phi y_{t-1}+u_t+\theta u_{t-1}\)\\
			ARIMA(1,1,0) & \texttt{'t'} & \(\diff y_t=\alpha_0+\phi\diff y_{t-1}+u_t\)\\
			ARIMA(1,1,1) & \texttt{'n'} & \(\diff y_t=\phi\diff y_{t-1}+u_t+\theta u_{t-1}\) \\
			ARIMA(1,2,0) & \texttt{'n'} & \(\diff^2 y_t=\phi\diff^2 y_{t-1}+u_t\)\\ \hline
		\end{tabular}
		\end{center} 
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\begin{tabular}{l|c}
			Модель & trend \\ \hline
			ARIMA(2,0,2) & \texttt{'ct'} \\
			ARIMA(2,1,0) & \texttt{'t'} \\
			ARIMA(2,1,1) & \texttt{'n'} \\
			ARIMA(1,2,0) & \texttt{'n'} \\ \hline
		\end{tabular}
		\end{center} 
		и постройте прогноз на 10 периодов. Значим ли снос/тренд?
		\item Проведите диагностику каждой модели.
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\begin{tabular}{l|c}
			Модель & trend \\ \hline
			ARIMA(3,0,2) & \texttt{'ct'} \\
			ARIMA(2,1,0) & \texttt{'t'} \\
			ARIMA(2,1,1) & \texttt{'n'} \\
			ARIMA(2,2,0) & \texttt{'n'} \\ \hline
		\end{tabular}
		\end{center} 
		и постройте прогноз на 10 периодов. Значим ли снос/тренд?
		\item Проведите диагностику каждой модели.
